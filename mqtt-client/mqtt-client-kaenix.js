...
// Marketplace installs only this file, without installing backend dependencies.
// Resolve lazily so an older backend can still load and display the plugin.
function getMqtt() {
    if (mqtt) return mqtt;
    try {
        mqtt = require('mqtt');
        ...
        try {
            ...
            throw new Error('MQTT-Bibliothek fehlt oder kann nicht geladen werden. Homeserver-Backend aktualisieren '
                + '(bei Quellcode-Installation: npm ci im Backend), Server neu starten und Plugins neu laden. '
                + 'Eine Plugin-Installation allein installiert keine Backend-Abhängigkeiten.');
        }
    }
    return mqtt;
}
...
function start(id, cfg, context) {
    ...
    s.client = mqttClient.connect({ protocol: 'mqtt', host: cfg.host, port: cfg.port,
